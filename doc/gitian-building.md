...
*Setup instructions for a gitian build of NachoCoin using a Debian VM or physical system.*
...
- [Building NachoCoin](#building-nachocoin)
...
Clone the git repositories for nachocoin and gitian and then checkout the nachocoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the nachocoin repository
...
To build NachoCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the nachocoin repository.
...
```bash
    ...
    From https://github.com/nachocoin/nachocoin
...
For example:
```bash
URL=https://github.com/konez2k/nachocoin.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/nachocoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/nachocoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/nachocoin-build.assert
```
...
[nachocoin/gitian.sigs](https://github.com/nachocoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
